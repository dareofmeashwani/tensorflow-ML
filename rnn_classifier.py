...
class model:
    ...
    def get_model(self,x,is_training):
        ...
        if isinstance(self.cell_size,list):
            rnn_layers = [tf.nn.rnn_cell.LSTMCell(self.cell_size[i],name=str(i)) for i in range(len(self.cell_size))]
            multi_rnn_cell = tf.nn.rnn_cell.MultiRNNCell(rnn_layers)
        elif isinstance(self.cell_size,int):
            rnn_layers = [tf.nn.rnn_cell.LSTMCell(self.cell_size, name=str(i)) for i in range(self.no_of_cell)]
            multi_rnn_cell = tf.nn.rnn_cell.MultiRNNCell(rnn_layers)

        outputs, states = tf.nn.dynamic_rnn(cell=multi_rnn_cell, inputs=x, dtype=tf.float32)

        op = tf.gather(outputs, int(outputs.get_shape()[1]) - 1, axis=1)
        op=ops.get_n_hidden_layers(op,'',self.hidden_layers,self.activation_list)
        return ops.get_hidden_layer(op,'output_layer',self.no_of_classes,'none')
