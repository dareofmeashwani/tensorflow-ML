...
class nnr:
    ...
    def __model_defination(self, x):
            ...
            with tf.name_scope('Hidden_Layer_' + str(i + 1)):
                if i == 0:
                    hidden_layer_weights_biases.update(
                        {'weights' + str(i): tf.Variable(
                            tf.random_normal([self.__feature_size, self.__hidden_layers[i]],mean=0.0,stddev=1.0), name='weight',),
                         'biases' + str(i): tf.Variable(tf.random_normal([1, self.__hidden_layers[i]],mean=0.0,stddev=1.0), name='biases')})
                    ...
                    hidden_layer_weights_biases.update(
                        {'weights' + str(i): tf.Variable(
                            tf.random_normal([self.__hidden_layers[i - 1], self.__hidden_layers[i]],mean=0.0,stddev=1.0), name='weight'),
                         'biases' + str(i): tf.Variable(tf.random_normal([1, self.__hidden_layers[i]],mean=0.0,stddev=1.0), name='biases')})
                    self.__variable_summaries(hidden_layer_weights_biases['weights' + str(i)], "weights")
                    self.__variable_summaries(hidden_layer_weights_biases['biases' + str(i)], "biases")
        ...
        with tf.name_scope('Output_Layer'):
            output_layer = {
                'weights': tf.Variable(tf.random_normal([self.__hidden_layers[len(self.__hidden_layers) - 1], 1],mean=0.0,stddev=1.0),
                                       name='weight'),
                'biases': tf.Variable(tf.random_normal([1],mean=0.0,stddev=1.0), name='biases')}
            ...
            #self.__choose_activation_function(logits,1)
        return  logits
    ...
    def pretrained_test(self, data):
        ...
        with tf.name_scope('Optimizer'):
            learningRate = tf.train.exponential_decay(learning_rate=self.__learning_rate, global_step=1,
                                                      decay_steps=self.__shape[0], decay_rate=0.97, staircase=True,
                                                      name='Learning_Rate')
