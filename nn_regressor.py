            ...
            with tf.name_scope('Hidden_Layer_'+str(i+1)):
                if i == 0:
                    hidden_layer_weights_biases.update(
                        {'weights' + str(i): tf.Variable(tf.random_normal([self.__feature_size, self.__hidden_layers[i]]),name='weight'),
                         'biases' + str(i): tf.Variable(tf.random_normal([1, self.__hidden_layers[i]]),name='biases')})
                    ...
                    hidden_layer_weights_biases.update(
                        {'weights' + str(i): tf.Variable(tf.random_normal([self.__hidden_layers[i - 1], self.__hidden_layers[i]]),name='weight'),
                         'biases' + str(i): tf.Variable(tf.random_normal([1, self.__hidden_layers[i]]),name='biases')})
                    self.__variable_summaries(hidden_layer_weights_biases['weights' + str(i)],"weights")
                    self.__variable_summaries(hidden_layer_weights_biases['biases' + str(i)],"biases")
